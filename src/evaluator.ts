...
async function runParallelStability(
  ...
  const results =
    evaluationRemoteBackend(config) === "modal"
      ? await Promise.all(
          commands.map(({ artifactPath: shardArtifactPath, command, shardCorpus }, index) =>
            runModalShard(
              config,
              task,
              repoPath,
      ...
      : await Promise.all(commands.map(({ command }) => runCommandStreaming(command, frontendPath)));
...
async function runModalShard(
  ...
  const bundlePath = join(artifactDir, `${corpus}-r${repeats}-shard-${shardIndex}.tgz`);
  createModalBundle(repoPath, bundlePath);

  const remoteArtifact = relative(frontendPath, shardArtifactPath);
  ...
  console.log(`\n[${task.id}] modal shard ${shardIndex + 1}: ${shardCorpus}`);
